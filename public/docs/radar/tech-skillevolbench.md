...
六个环境（代码修改、API 编排、数据处理、文档转换、研究合成、沟通操作），每个五个任务家族，每个家族六个角色任务。**三个学习任务**：标准任务采集经验 → 升级版暴露缺失的子能力 → 变体版改变形式保持底层逻辑，每步 Agent 可选写/改/不改技能。**三个冻结评估任务**：上下文漂移（技能需正确识别但描述变了）、对抗性捷径（存在绕开正确流程的浅层方案）、组合（需与其他技能协同），评估时技能库冻结——测的是"学到的能不能迁移"而非"现场再学"。
