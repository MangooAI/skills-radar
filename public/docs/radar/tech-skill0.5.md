...
每个训练 step，Agent 先用标准配置（仅带任务特定技能）对每个任务跑 G 条轨迹，算出通过率 $p_i$ 后分三桶：**困难任务**（$p_i=0$，全零 reward）、**中等任务**（$0 < p_i \le \eta_t$）、**简单任务**（$p_i > \eta_t$）。门槛 $\eta_t$ 是过去 W 个 step 的 batch 级通过率滑动均值，随 Agent 整体水平动态调整。三种损失互斥——一个任务只触发一条优化路径。
