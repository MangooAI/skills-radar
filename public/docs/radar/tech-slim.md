# SLIM：让 Agent 自己决定该"记住"还是"忘记"哪些技能

**一句话描述**：SLIM 把活跃技能集当作一个**动态优化变量**——每个审计周期用 leave-one-skill-out 对照实验计算每个技能的**边际外部贡献（MEC）**，高的保留、低的退役、不够用就扩展，全程不让技能数膨胀，在 ALFWorld 上比 SkillRL 高出 **12.5 个百分点**。
...
3. [详解](https://zhuanlan.zhihu.com/p/2041216739745194889)
