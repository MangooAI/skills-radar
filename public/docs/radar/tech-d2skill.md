...
1. 事后技能效用更新：技能组与基线组的成功率差值作为任务级技能效用信号；
2. 事后内在奖励塑形：技能注入轨迹表现优于基线组时获得额外奖励；
3. 策略优化：将内在奖励融入总回报，基于 GRPO 算法完成策略更新。
...
1. 第一阶段：根据语义相似度检索 top-m 候选技能；
2. 第二阶段：结合归一化语义相似度、技能效用和 UCB 探索项的综合分数排序，选出 top-k 技能注入策略上下文。同时通过效用导向的定期剪枝防止技能库无限膨胀，新生成的技能享有保护期以确保充分评估。
