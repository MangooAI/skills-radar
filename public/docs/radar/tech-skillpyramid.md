...
- **粗分组阶段**：仅看技能名称和简短描述，将功能相近、有潜在复用可能的技能归入同组；
- **细分析阶段**：打开每组内技能的完整内容，深入识别具体关系类型，输出结构化的 **关系构建任务**，指定哪些技能之间存在什么关系及其证据。
...
- **向下原子提取**：从一组技能中提取共享的最小可复用能力（如"定位并获取目标物体"），作为原子技能放入金字塔更低层，同时重写原始技能，在关键步骤显式引用原子技能。
- **向上抽象归纳**：从一组技能中提取共享的高层解题模式（如"获取→处理→验证→放置"），作为抽象技能放入金字塔更高层，提供可泛化的任务框架而非僵化步骤。两种操作共同将平铺技能库重建为有层级、有引用、有方向的图结构。
